%女声变男声
function Y=voicel(x)
[x,fs]=audioread('aa.wav'); %获取女声语音信号
%显示频谱及波形
xt=x(:,1);
figure(1);
tend=length(xt)/fs;
...
title('语音信号song2');xlabel('t(s)');ylabel('y');
XT=fft(xt);
Ts=t(2)-t(1);
ws=1/Ts;
wn=ws/2;
...
XT=abs(XT(1:1:tnum));
subplot(2,1,2);plot(w,XT);axis([0 2000 0 1000]);grid;
title('语音信号song2的频谱');xlabel('f (Hz)');ylabel('Y');
%
%变声
d=resample(x,3,2); %重采样，更改采样率，降低基频
%时长规整，使语音信号恢复为原来的时长
...
for i=1:2%由于读取的语音信号为2维，降维后会有信息的丢失，所以做二次循环，对两个维度分别处理
...
title('变声后语音信号song2');xlabel('t(s)');ylabel('y');
YT=fft(yt);
Tsy=ty(2)-ty(1);
wsy=1/Tsy;
wny=wsy/2;
...
YT=abs(YT(1:1:tnumy));
subplot(2,1,2);plot(wy,YT);axis([0 2000 0 1000]);grid;
title('变声后语音信号song2的频谱');xlabel('f (Hz)');ylabel('Y');
%
sound(Y,fs);
end